...
try:
    ...
    def PrintPackingSticker(pdf, rowSKU, rowCH, rowDate, listSKU, pad_x, pad_y):
        if rowSKU.get() == '':
            return
        readedIndex = listSKU.index(rowSKU.get())+2
        ...
        if cherVarStickerButtonA4.get() == FALSE:
            pdf.add_page()

        pdf.add_font('arial', '', 'fonts/arial.ttf', uni=True)
        pdf.set_font('arial', '', 9)

        pdf.cell(w=68, h=45, border=0, ln=0)
        pdf.cell(w=0, h=5, border=0, ln=2)
        ...
        ###
        pdf.set_xy(pad_x+1, pad_y+60)
        pdf.cell(w=68, h=6.0, txt=raw, border=0, ln=1)
        pdf.set_xy(pad_x+1, pad_y+65)
        pdf.cell(w=0, h=10.0, txt=descrUA, ln=1)

        pdf.add_font('arialB', '', 'fonts/arial_black.ttf', uni=True)
        pdf.set_font('arialB', '', size=19)
        pdf.set_xy(pad_x+1, pad_y+75)
        pdf.write(txt=descrCODE)

        pdf.image("img\ecosoft.png", x=2+pad_x, y=3+pad_y, w=65.06396)

        pdf.image("img\EAC.png", x=34+pad_x, y=48+pad_y, h=6.25615)

        pdf.image("img\CE.png", x=10+pad_x, y=48+pad_y, h=6.881765)

        pdf.image("img/ukrsepro.png", x=56+pad_x, y=48+pad_y, h=11.26107)

        pdf.image(f"img\{rowSKU.get()}.png", x=73 +
                  pad_x, y=29+pad_y, w=28.152675)
    ...
    def PrintGuaranteeSticker(pdf, rowSKU, rowCH, rowDate):
        ...
        pdf.cell(txt=model_SKU, align='C', w=0, h=9, border=0, ln=2)
        pdf.cell(txt=f'CH-{serial_number.get()}',
                 align='C', w=0, h=9, border=0, ln=2)
        pdf.cell(txt=f'Date: {rowDate.get()}', w=0, h=9, border=0, align='C')
    ...
    def PackingStikers_12pcs():
        ...
        if cherVarStickerButtonA4.get() == TRUE:
            print(f'Галка ввімкнета, треба друкувати на А4')
            move_x_0 = 105
            move_x_1 = 72
            move_y_0 = 90
            move_y_1 = 46.5
            ...
            PrintPackingSticker(pdf, sku_variable,
                                serial_variable, date_variable, SKU, 3, 0)
            PrintPackingSticker(pdf, sku_variable_2,
                                serial_variable_2, date_variable_2, SKU, move_x_0, 0)
            PrintPackingSticker(pdf, sku_variable_3,
                                serial_variable_3, date_variable_3, SKU, 3, move_y_0)
            PrintPackingSticker(pdf, sku_variable_4,
                                serial_variable_4, date_variable_4, SKU, move_x_0, move_y_0)
            PrintPackingSticker(pdf, sku_variable_5,
                                serial_variable_5, date_variable_5, SKU, 3, move_y_0*2+10)
            PrintPackingSticker(pdf, sku_variable_6,
                                serial_variable_6, date_variable_6, SKU, move_x_0, move_y_0*2)
            PrintPackingSticker(pdf, sku_variable_7,
                                serial_variable_7, date_variable_7, SKU, 0, 0)
            PrintPackingSticker(pdf, sku_variable_8,
                                serial_variable_8, date_variable_8, SKU, 105, 0)
            PrintPackingSticker(pdf, sku_variable_9,
                                serial_variable_9, date_variable_9, SKU, 0, 0)
            PrintPackingSticker(pdf, sku_variable_10,
                                serial_variable_10, date_variable_10, SKU, 105, 0)
            PrintPackingSticker(pdf, sku_variable_11,
                                serial_variable_11, date_variable_11, SKU, 0, 0)
            PrintPackingSticker(pdf, sku_variable_12,
                                serial_variable_12, date_variable_12, SKU, 105, 0)
            pdf.output("Стікери упаковки.pdf")
            return

        pdf = fpdf.FPDF('P', 'mm', (104, 86))
        PrintPackingSticker(pdf, sku_variable,
                            serial_variable, date_variable, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_2,
                            serial_variable_2, date_variable_2, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_3,
                            serial_variable_3, date_variable_3, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_4,
                            serial_variable_4, date_variable_4, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_5,
                            serial_variable_5, date_variable_5, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_6,
                            serial_variable_6, date_variable_6, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_7,
                            serial_variable_7, date_variable_7, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_8,
                            serial_variable_8, date_variable_8, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_9,
                            serial_variable_9, date_variable_9, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_10,
                            serial_variable_10, date_variable_10, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_11,
                            serial_variable_11, date_variable_11, SKU, 0, 0)
        PrintPackingSticker(pdf, sku_variable_12,
                            serial_variable_12, date_variable_12, SKU, 0, 0)
        pdf.output("Стікери упаковки.pdf")
    ...
    ###
    def FunctionStickerButtonA4():
        print(f'Положення кнопки: {cherVarStickerButtonA4.get()}')
    ###
    ...
    # Створення змінної для галки А4, кнопки "Стікери упаковки"
    cherVarStickerButtonA4 = IntVar(value=0)
    ...
    Button(win, text='Зберегти шильди', command=PrintShields_12pcs).grid(
        row=2, column=5, ipadx=27, ipady=0, padx=10, pady=0)
    Button(win, text='Зберегти стікери гарантії ', command=PrintGuaranteeStikers_12pcs
           ).grid(row=3, column=5, ipadx=5, ipady=0, padx=0, pady=0)
    Button(win, text='Зберегти стікери упаковки', command=PrintPackingStikers_12pcs
           ).grid(row=4, column=5, ipadx=2, ipady=0, padx=0, pady=0)

    Button(win, text='Зберегти все', command=SaveAllStickers).grid(
        row=6, column=5, ipadx=40, ipady=0, padx=0, pady=0)

    Checkbutton(text='A4', variable=cherVarStickerButtonA4, command=FunctionStickerButtonA4).grid(
        row=4, column=6)

    #   Напис "розробник"
    tk.Label(win, text='made by maksym.protsak@gmail.com V1.1',
             bg='#EBEBEB', fg='#808080').place(x=0, y=550)
    #grid(row=14, column=5, rowspan=6, sticky='ES')
